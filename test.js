...
async function run() {
  const secretKey = fs.readFileSync("/home/Abhi/hii.json", "utf8");
  ...
  const secretKey2 = fs.readFileSync("./hi.json", "utf8");
  ...
  const contract = new PublicKey(
    "AFLwi1VLdGgtHYmxdg2EeqkYvv2oMWwJE4FpTbQfroL1"
  );
  ...
  const idl = await anchor.Program.fetchIdl(contract, provider);
  // console.log(idl)

  const program = new anchor.Program(idl, contract, provider);
  ...
  const vaultAccount = await PublicKey.findProgramAddress(
    [Buffer.from("vault"), beneficiary.toBuffer()],
    contract
  );
